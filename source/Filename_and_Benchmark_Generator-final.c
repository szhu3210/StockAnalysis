//
//  Filename and Benchmark Generator.c
//  Phase 29
//
...
void initialfile(char filenames[][255],int *filenum)
{
    ...
    {
        sprintf(filename, "SZ#%d.txt", i);				//adjust the file name
        filename[3] = '0';
        sprintf(a, "%s%s", folder, filename);			//create the file directory
        t = fopen(a, "r");
        if (t != NULL)
...
int main()
{
    ...
    for (k=0;k<BMN;k++)
    {
        ...
        //set output path
        sprintf(outpath,"/home/troy/Stock Analysis/1Data/5stockdata-final/Test0/filename%d.txt",k);
        out=fopen(outpath,"w");
